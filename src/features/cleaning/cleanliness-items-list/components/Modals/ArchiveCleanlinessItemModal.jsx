...
export function ArchiveCleanlinessItemModal({
    open,
    onOpenChange,
    item,
    onConfirm,
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            let errorMessage = "An error occurred while archiving the item";
            
            if (err.response?.status === 403) {
                errorMessage = "You don't have permission to archive this item. Contact your administrator if you believe this is an error.";
            } else if (err.response?.data) {
                const errorData = err.response.data;
                if (errorData.errors) {
                    const fieldErrors = Object.values(errorData.errors);
                    errorMessage = fieldErrors[0] || errorData.message || errorMessage;
            ...
            >
                <ModalHeader className="pb-4 md:pb-3 lg:pb-3.5! xl:pb-4.5! 2xl:pb-5! 3xl:pb-6!">
                    <ModalTitle className="text-lg lg:text-xs! xl:text-sm! 2xl:text-md! 3xl:text-lg! font-semibold text-center">
                        {isBulk ? "Archive Cleanliness Items" : "Archive Item"}
                    </ModalTitle>
                </ModalHeader>
                ...
                <div className="flex flex-col items-center space-y-4 md:space-y-6">
                    ...
                    <div className="flex flex-col items-center gap-2">
                        <p className="px-4 md:px-4 lg:px-4.5! xl:px-5.5! 2xl:px-6.5! 3xl:px-8! text-center text-sm md:text-[8px] lg:text-[8.5px]! xl:text-[11.5px]! 2xl:text-[13px]! 3xl:text-base! font-medium text-base-color">
                            {isBulk
                                ? "Are you sure you want to archive these cleanliness items?"
                        ...
                        {item && (
                            <p className="px-4 text-center font-medium text-foreground text-xs md:text-[7px] lg:text-[8px]! xl:text-[10px]! 2xl:text-xs! 3xl:text-sm!">
                                {isBulk
                                    ? `${item.length} cleanliness item(s) selected`
                                    : ""
                                }
                            </p>
                        )}
                        <p className="px-4 md:px-4 lg:px-4.5! xl:px-5.5! 2xl:px-6.5! 3xl:px-8! text-center text-xs md:text-[7px] lg:text-[8px]! xl:text-[10px]! 2xl:text-xs! 3xl:text-sm! text-lighter-text">
                            Archiving this item means it will be hidden from active views. You can restore it later if needed.
                        </p>
                    </div>
