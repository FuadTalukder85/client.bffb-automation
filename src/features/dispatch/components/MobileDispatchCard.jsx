...
export default function MobileDispatchCard({
  ...
  const canArchive = hasPermission(permissions, PERMISSIONS.DISPATCH.DELETE);
  const { isSelected, isSelectionMode, pressHandlers } = useMobileSelection({
    itemId: record._id || record.id,
  ...
  return (
    <div
      {...pressHandlers}
      className={cn(
        "relative w-full mb-4 select-none cursor-pointer rounded-2xl transition-all duration-200",
        isSelected ? "scale-[0.99] shadow-lg" : ""
      )}
    >
      {isSelected && (
        <div className="absolute inset-0 rounded-2xl pointer-events-none border border-primary bg-primary/[0.06] z-10 animate-in fade-in duration-200" />
      )}
      <ExpandableCard
        className={cn(
          "bg-background rounded-2xl border border-table-stroke/30 shadow-sm overflow-hidden w-full",
          className
        )}
      >
