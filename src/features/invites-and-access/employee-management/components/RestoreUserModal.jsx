...
import { useRestoreUser } from "@/hooks/mutations";

export function RestoreUserModal({ open, onOpenChange, user, onSuccess, className }) {
  const [error, setError] = useState(null);
  const { mutateAsync: restoreUser, isPending: isLoading } = useRestoreUser();
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || "Failed to restore user. Please try again.";
    ...
    <Modal open={open} onOpenChange={handleClose}>
      ...
      >
        <ModalHeader className="flex flex-col items-center justify-center mb-4 space-y-4 text-center">
          ...
          >
            <FaUserPlus className="w-16 h-16 text-nav-highlight" />
          </motion.div>
          ...
          <ModalDescription className="text-sm md:text-[8px] lg:text-[8.5px]! xl:text-[11.5px]! 2xl:text-[13px]! 3xl:text-base! font-medium text-center text-lighter-text">
            <span className="font-medium text-base-color">
              Are you sure you want to restore <span className="text-primary">{userName}</span>?
            </span>
            <br />
            Make sure to only perform this function with proper authorization.
            They may have left their position at BFF.
          </ModalDescription>
        </ModalHeader>

        {error && (
          <div className="px-4 py-2 mb-4 text-sm text-center text-red-600 bg-red-50 rounded-lg">
            {error}
          </div>
        )}
