...
import { useUnblockUser } from "@/hooks/mutations";

export function UnblockUserModal({ open, onOpenChange, user, onSuccess, className }) {
  const [error, setError] = useState(null);
  const { mutateAsync: unblockUser, isPending: isLoading } = useUnblockUser();
    ...
    <Modal open={open} onOpenChange={handleClose}>
      ...
      >
        <ModalHeader className="flex flex-col items-center justify-center mb-4 space-y-4 text-center">
          <ModalTitle className="text-lg font-semibold">Unblock User?</ModalTitle>
          ...
          >
            <MdLockOpen className="w-16 h-16 text-nav-highlight" />
          </motion.div>
          ...
          <ModalDescription className="text-sm md:text-[8px] lg:text-[8.5px]! xl:text-[11.5px]! 2xl:text-[13px]! 3xl:text-base! font-medium text-center text-lighter-text">
            <span className="font-medium text-base-color">
              Unblock <span className="text-primary">{userName}</span> to allow login again.
            </span>
            <br />
