...
export function CategoryModal({
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            setError(getApiErrorMessage(err, "An error occurred while saving the category"));
        } finally {
            setIsLoading(false);
        }
    };
            ...
            >
                <ModalHeader className="mb-4 lg:mb-2 xl:mb-2.5 2xl:mb-3 3xl:mb-4">
                    ...
                    <ModalDescription className="sr-only">
                        {mode === "create" ? "Create a new category" : "Update an existing category"}
                    </ModalDescription>
                </ModalHeader>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 lg:space-y-2 xl:space-y-2.5 2xl:space-y-3 3xl:space-y-4">
                    ...
                    <div className="space-y-1">
                        <label className="text-sm lg:text-[8px] xl:text-[10px] 2xl:text-xs 3xl:text-sm font-medium text-base-color">
                            <p className="mt-1 lg:my-[1px] xl:my-[2px] 2xl:my-[3px] 3xl:my-1">Category Code <span className="text-red-500">*</span>
                            </p>
                        </label>
                        <Input
                            {...register("categoryCode", { required: "Code is required" })}
                            placeholder="Enter category code (e.g., 9999)"
                    ...
                    <div className="space-y-1">
                        <label className="text-sm lg:text-[8px] xl:text-[10px] 2xl:text-xs 3xl:text-sm font-medium text-base-color leading-none">
                            <p className="mt-1 lg:my-[1px] xl:my-[2px] 2xl:my-[3px] 3xl:my-1">Category Name <span className="text-red-500">*</span>
                            </p>
                        </label>
                        <Input
                            {...register("name", { required: "Name is required" })}
                            placeholder="Enter category name (e.g., Bakery)"
                            className="w-full text-sm lg:text-[8px] xl:text-[10px] 2xl:text-xs 3xl:text-sm border rounded-lg border-table-stroke focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
...
export function ArchiveCategoryModal({
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            setError(getApiErrorMessage(err, "An error occurred while archiving the category"));
        } finally {
            setIsLoading(false);
        }
    };

    const isBulk = Array.isArray(category);
            ...
            >
                <ModalHeader className="pb-4 md:pb-3 lg:pb-3.5! xl:pb-4.5! 2xl:pb-5! 3xl:pb-6!">
                    <ModalTitle className="text-lg lg:text-xs! xl:text-sm! 2xl:text-md! 3xl:text-lg! font-semibold text-center">
                        {isBulk ? "Archive Categories" : "Archive Category"}
                    </ModalTitle>
                </ModalHeader>
                ...
                <div className="flex flex-col items-center space-y-4 md:space-y-6">
                    ...
                    <div className="flex flex-col items-center gap-2">
                        <p className="px-4 md:px-4 lg:px-4.5! xl:px-5.5! 2xl:px-6.5! 3xl:px-8! text-center text-sm md:text-[8px] lg:text-[8.5px]! xl:text-[11.5px]! 2xl:text-[13px]! 3xl:text-base! font-medium text-base-color">
                            {isBulk
                                ? "Are you sure you want to archive these categories?"
                                : <>Are you sure you want to archive category <span className="font-bold">"{category?.name}"</span>?</>
                            }
                        </p>
                        {category && (
                            <p className="px-4 text-center font-medium text-foreground text-xs md:text-[7px] lg:text-[8px]! xl:text-[10px]! 2xl:text-xs! 3xl:text-sm!">
                                {isBulk
                                    ? `${category.length} category(ies) selected`
                                    : ""
                                }
                            </p>
                        )}
                        <p className="px-4 md:px-4 lg:px-4.5! xl:px-5.5! 2xl:px-6.5! 3xl:px-8! text-center text-xs md:text-[7px] lg:text-[8px]! xl:text-[10px]! 2xl:text-xs! 3xl:text-sm! text-lighter-text">
                            Archiving this category means sub-categories and sub-sub-categories under it will also be affected. Proceed with caution.
                        </p>
                    </div>
...
export function RestoreCategoryModal({
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            setError(getApiErrorMessage(err, "An error occurred while restoring the category"));
        } finally {
            setIsLoading(false);
        }
    };
