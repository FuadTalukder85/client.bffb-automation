...
import { Trash2, Pencil, LayoutGrid, Tag } from "lucide-react";
...
export default function MobileSubCategoryCard({
  subCategory,
  serialNumber,
  onEdit,
  onArchive,
  ...
  const { isSelected, isSelectionMode, pressHandlers } = useMobileSelection({
    itemId: subCategory._id || subCategory.id,
    selectedIds: selectedRowIds,
    onSelectChange,
    canSelect: canArchive && isActive,
  });

  // Define button configurations for active subcategories
  const activeSubCategoryButtons = [
    {
      key: "viewChildren",
      icon: (props) => <LayoutGrid {...props} className={cn("action-button-icon", props.className)} />,
    ...
    {
      key: "viewTags",
      icon: (props) => <Tag {...props} className={cn("action-button-icon", props.className)} />,
      onClick: () => onViewTags?.(subCategory),
      title: "Tags",
      className:
        "rounded-none flex-1 text-[#552e8e] dark:text-white hover:bg-primary/10 bg-transparent",
    },
    {
      key: "edit",
    ...
    },
    {
      ...
      title: "Delete",
      className:
        "rounded-l-none flex-1 text-[#552e8e] dark:text-white hover:bg-primary/10 bg-transparent",
    },
  ];
  ...
  return (
    ...
    >
      ...
      {/* Footer with action buttons - only for active subcategories */}
      {isActive && !isSelectionMode && (
        <ButtonGroup
          buttons={activeSubCategoryButtons}
          className="px-4 pt-2 pb-4 w-40"
          fullWidth
          gap="gap-0"
        />
      )}
