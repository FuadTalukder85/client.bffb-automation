...
export function SubCategoryModal({
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            setError(getApiErrorMessage(err, "An error occurred while saving the sub-category"));
        } finally {
            setIsLoading(false);
        }
    };
            ...
            >
                <ModalHeader className="mb-4 lg:mb-2 xl:mb-2.5 2xl:mb-3 3xl:mb-4">
                    ...
                    <ModalDescription className="sr-only">
                        {mode === "create" ? "Create a new sub-category" : "Update an existing sub-category"}
                    </ModalDescription>
                </ModalHeader>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 lg:space-y-2 xl:space-y-2.5 2xl:space-y-3 3xl:space-y-4">
                    ...
                    <div className="space-y-1">
                        <label className="text-sm lg:text-[8px] xl:text-[10px] 2xl:text-xs 3xl:text-sm font-medium text-base-color">
                            <p className="mt-1 lg:my-[1px] xl:my-[2px] 2xl:my-[3px] 3xl:my-1">Sub-category Code <span className="text-red-500">*</span>
                            </p>
                        </label>
                        <Input
                            {...register("subCategoryCode", { required: "Code is required" })}
                    ...
                    <div className="space-y-1">
                        <label className="text-sm lg:text-[8px] xl:text-[10px] 2xl:text-xs 3xl:text-sm font-medium text-base-color leading-none">
                            <p className="mt-1 lg:my-[1px] xl:my-[2px] 2xl:my-[3px] 3xl:my-1">Sub-category Name <span className="text-red-500">*</span>
                            </p>
                        </label>
                        <Input
                    ...
                    <ModalFooter className="flex-row gap-4 lg:gap-1 xl:gap-2 2xl:gap-3 3xl:gap-4 mt-6 text-xs lg:text-[8px] xl:text-[10px] 2xl:text-xs 3xl:text-sm sm:justify-between h-9 lg:h-5 xl:h-6 2xl:h-7 3xl:h-9">
                        ...
                        >
                            {isLoading ? (
                                ...
                                mode === "create" ? "Create Sub-category" : "Save Changes"
                            )}
                        </Button>
                    </ModalFooter>
                </form>
...
export function ArchiveSubCategoryModal({
    open,
    onOpenChange,
    subCategory,
    onConfirm,
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            setError(getApiErrorMessage(err, "An error occurred while archiving the sub-category"));
        } finally {
            setIsLoading(false);
        }
    };
...
export function RestoreSubCategoryModal({
    open,
    onOpenChange,
    subCategory,
    onConfirm,
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            setError(getApiErrorMessage(err, "An error occurred while restoring the sub-category"));
        } finally {
            setIsLoading(false);
        }
    };
            ...
            >
                ...
                <div className="flex flex-col items-center space-y-4 md:space-y-6">
                    <motion.div
                        ...
                        className="flex items-center justify-center w-16 md:w-10 lg:w-13! xl:w-17! 2xl:w-19! 3xl:w-24! h-16 md:h-10 lg:h-13! xl:h-17! 2xl:h-19! 3xl:h-24!"
                    >
                        <div className="flex items-center justify-center w-16 h-16 text-white rounded-full shadow-lg md:w-24 md:h-24 bg-invite-status-text shadow-purple-900/20">
