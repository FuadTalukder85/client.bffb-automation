...
export function SubSubCategoryModal({
    open,
    onOpenChange,
    subSubCategory,
    onConfirm,
    ...
    useEffect(() => {
        if (subSubCategory && mode === "update") {
            reset({
                name: subSubCategory.name,
                subSubCategoryCode: subSubCategory.subSubCategoryCode,
            });
        } else {
            reset({
                name: "",
        ...
        setError(null);
    }, [subSubCategory, mode, reset, open]);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            setError(getApiErrorMessage(err, "An error occurred while saving the sub-sub-category"));
        } finally {
            setIsLoading(false);
        }
    };
            ...
            >
                <ModalHeader className="mb-4 lg:mb-2 xl:mb-2.5 2xl:mb-3 3xl:mb-4">
                    ...
                    <ModalDescription className="sr-only">
                        {mode === "create" ? "Create a new sub-sub-category" : "Update an existing sub-sub-category"}
                    </ModalDescription>
                </ModalHeader>

                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 lg:space-y-2 xl:space-y-2.5 2xl:space-y-3 3xl:space-y-4">
                    {/* Sub-sub-category Code Input */}
                    <div className="space-y-1">
                        <label className="text-sm lg:text-[8px] xl:text-[10px] 2xl:text-xs 3xl:text-sm font-medium text-base-color">
                            <p className="mt-1 lg:my-[1px] xl:my-[2px] 2xl:my-[3px] 3xl:my-1">Sub-sub-category Code <span className="text-red-500">*</span>
                            </p>
                        </label>
                        <Input
                            {...register("subSubCategoryCode", { required: "Code is required" })}
                            placeholder="Enter sub-sub-category code"
                            ...
                            error={errors.subSubCategoryCode?.message}
                        />
                    </div>

                    {/* Sub-sub-category Name Input */}
                    <div className="space-y-1">
                        <label className="text-sm lg:text-[8px] xl:text-[10px] 2xl:text-xs 3xl:text-sm font-medium text-base-color leading-none">
                            <p className="mt-1 lg:my-[1px] xl:my-[2px] 2xl:my-[3px] 3xl:my-1">Sub-sub-category Name <span className="text-red-500">*</span>
                            </p>
                        </label>
                        <Input
                    ...
                    <ModalFooter className="flex-row gap-4 lg:gap-1 xl:gap-2 2xl:gap-3 3xl:gap-4 mt-6 text-xs lg:text-[8px] xl:text-[10px] 2xl:text-xs 3xl:text-sm sm:justify-between h-9 lg:h-5 xl:h-6 2xl:h-7 3xl:h-9">
                        ...
                        >
                            {isLoading ? (
                                ...
                                mode === "create" ? "Create Sub-sub-category" : "Save Changes"
                            )}
                        </Button>
                    </ModalFooter>
                </form>
...
export function ArchiveSubSubCategoryModal({
    open,
    onOpenChange,
    subSubCategory,
    onConfirm,
    ...
    const handleConfirm = async () => {
        ...
        setError(null);
        try {
            await onConfirm(subSubCategory);
            handleClose(false);
        } catch (err) {
            setError(getApiErrorMessage(err, "An error occurred while archiving the sub-sub-category"));
        } finally {
            setIsLoading(false);
        }
    };
...
export function RestoreSubSubCategoryModal({
    open,
    onOpenChange,
    subSubCategory,
    onConfirm,
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            setError(getApiErrorMessage(err, "An error occurred while restoring the sub-sub-category"));
        } finally {
            setIsLoading(false);
        }
    };
                ...
                <div className="flex flex-col items-center space-y-4 md:space-y-6">
                    ...
                    <p className="px-4 md:px-4 lg:px-4.5! xl:px-5.5! 2xl:px-6.5! 3xl:px-8! text-center text-sm md:text-[8px] lg:text-[8.5px]! xl:text-[11.5px]! 2xl:text-[13px]! 3xl:text-base! font-medium leading-relaxed text-muted-foreground">
                        <span className="font-medium text-foreground">
                            Are you sure you want to restore this sub-sub-category?
                        </span>
                        <br />
