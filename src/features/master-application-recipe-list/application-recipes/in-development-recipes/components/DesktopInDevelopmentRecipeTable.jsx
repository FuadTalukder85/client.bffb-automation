...
export default function DesktopInDevelopmentRecipeTable({
  recipes,
  onCellEdit,
  refetch,
  currentPage = 1,
  ...
  const canDownloadRecipe = hasPermission(
    permissions,
    PERMISSIONS.RECIPE.EXPORT_IN_DEVELOPMENT_RECIPES
  );
        ...
        return tags && tags.length > 0 ? (
          <div className="flex flex-wrap justify-center gap-1">
            {tags.slice(0, 3).map((tag, index) => (
              <span key={index} className="table-tag-pill">
                {tag.name}
              </span>
            ))}
            {tags.length > 3 && (
            ...
            {isActive ? (
              <>
                ...
                <button
                  ...
                  title="Archive"
                  aria-label="Archive"
                  className="action-button flex items-center justify-center gap-1.5 text-sm font-semibold rounded-r-md rounded-l-none text-nav-highlight hover:bg-purple-200 border border-primary-shade-2 transition-colors focus-visible:z-10 focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
                >
