...
import { Eye, Search, X, Loader2, ChevronRight, CookingPot } from "lucide-react";
import { bffProductCodeService } from "@/services/bffProductCodeService";

const MAX_VISIBLE_RECIPES = 2;

export function AppliedRecipesList({ productCodeId, referenceRecipes, className }) {
  ...
  // Use referenceRecipes from the product detail response as the initial data source
  useEffect(() => {
    if (Array.isArray(referenceRecipes)) {
      setRecipes(referenceRecipes);
    }
  }, [referenceRecipes]);

  // When the modal opens, fetch full data if not already loaded from referenceRecipes
  useEffect(() => {
    if (!modalOpen || !productCodeId) {
    ...
    const fetchRecipes = async () => {
      setIsLoading(true);
      try {
        const response = await bffProductCodeService.getRecipesUsingProduct(productCodeId);
        setRecipes(response.data || response || []);
      } catch (error) {
        console.error("Failed to fetch applied recipes:", error);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    fetchRecipes();
  }, [modalOpen, productCodeId, recipes.length]);

  const handleNavigateToRecipe = (recipe) => {
    const recipeId = recipe.id || recipe._id;
    if (recipeId) {
      navigate(`/application-lab/application-recipes/${recipeId}`);
    }
  };
  ...
  const visibleRecipes = recipes.slice(0, MAX_VISIBLE_RECIPES);
  const hasMore = recipes.length > MAX_VISIBLE_RECIPES;
        ...
        <div>
          ...
          {recipes.length > 0 ? (
            ...
            <p className="text-xs text-muted-foreground italic py-1">
              No recipes applied yet.
            </p>
          )}
        </div>
      </div>

      {/* All Applied Recipes Modal */}
          ...
          <div className="md:mt-6 md:mx-4">
            ...
            <ModalHeader className="pb-2">
              ...
              <ModalDescription className="sr-only">
                All recipes where this product code has been applied
              </ModalDescription>
            </ModalHeader>
            ...
            <p className="text-xs lg:text-[7.5px] xl:text-[10px] 2xl:text-xs 3xl:text-sm text-lighter-text mb-3 lg:mb-2 xl:mb-2.5 2xl:mb-3 3xl:mb-4">
              Showing all {recipes.length} recipe(s) using this product code
            </p>
              ...
              <div className="max-h-[280px] lg:max-h-[160px] xl:max-h-[200px] 2xl:max-h-[240px] 3xl:max-h-[320px] overflow-y-auto custom-scrollbar">
                {isLoading ? (
                  ...
                  filteredRecipes.map((recipe, index) => (
                    <div
                      key={recipe.id || recipe._id || `${recipe.recipeCode}-${index}`}
