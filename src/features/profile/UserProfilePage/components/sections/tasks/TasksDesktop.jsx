...
const TabButton = ({ id, label, isActive, count, onClick }) => {
  return (
    <button
      onClick={() => onClick(id)}
      className={`relative 3xl:w-[140px] 2xl:w-[120px] xl:w-[100px] lg:w-[80px] shrink-0 3xl:px-2 2xl:px-1.5 xl:px-1 lg:px-0.5 3xl:py-2 2xl:py-1.5 xl:py-1 lg:py-0.5 3xl:text-[14px] 2xl:text-[12px] xl:text-[10px] lg:text-[8px] font-semibold transition-colors ${
        isActive
    ...
    >
      ...
      {typeof count === "number" && count > 0 ? (
        <span className="absolute top- right-.5 3xl:min-w-3.5 2xl:min-w-3 lg:min-w-2 3xl:h-3.5 2xl:h-3 lg:h-2.5 3xl:px-1 2xl:px-0.5 xl:px-0.5 lg:px-0.5 rounded-md bg-primary text-white 3xl:text-[10px] 2xl:text-[9px] xl:text-[8px] lg:text-[7px] 3xl:leading-4 2xl:leading-3.5 xl:leading-3 lg:leading-2.5 font-medium">
          {count}
        </span>
      ) : null}
    </button>
        ...
        {options.map((option) => {
          ...
          return (
            <button
              ...
              className={`3xl:px-4 2xl:px-3.5 xl:px-3 lg:px-2.5 3xl:py-2 2xl:py-1.5 xl:py-1 lg:py-0.5 3xl:text-[14px] 2xl:text-[12px] xl:text-[10px] lg:text-[8px] font-medium transition-colors border-b-2 whitespace-nowrap ${
                isSelected ? "" : "border-transparent text-lighter-text hover:text-foreground"
              }`}
              style={{
...
const TasksDesktop = () => {
  ...
  const projectTaskColumns = [
    ...
    },
    {
      ...
      enablePinning: true,
      cell: ({ row }) => (
        <div className="flex items-center justify-center 3xl:gap-2 2xl:gap-1.5 xl:gap-1 lg:gap-0.5">
          <button onClick={() => handleOpenTaskAction(row.original)} className="3xl:size-8 2xl:size-6.5 xl:size-5.5 lg:size-4.5 3xl:rounded-md 2xl:rounded-md xl:rounded-sm lg:rounded-sm border border-nav-highlight/20 bg-primary-shade-2 text-nav-highlight inline-flex items-center justify-center">
            <ExternalLink className="3xl:w-4 3xl:h-4 2xl:w-3.5 2xl:h-3.5 xl:w-3 xl:h-3 lg:w-2.5 lg:h-2" />
          </button>
          <button onClick={() => handleOpenStatusModal(row.original, TASK_MODAL_TYPES.PROJECT)} className="3xl:size-8 2xl:size-6.5 xl:size-5.5 lg:size-4.5 3xl:rounded-md 2xl:rounded-md xl:rounded-sm lg:rounded-sm border border-nav-highlight/20 bg-background text-nav-highlight inline-flex items-center justify-center">
            <AiOutlineFileDone className="3xl:w-4 3xl:h-4 2xl:w-3.5 2xl:h-3.5 xl:w-3 xl:h-3 lg:w-2.5 lg:h-2" />
          </button>
        </div>
      ),
  ...
  const internalTaskColumns = [
      ...
      { id: "actions", header: "Actions", enablePinning: true, cell: ({ row }) => (
          <div className="flex items-center justify-center gap-2">
            <button onClick={() => handleOpenInternalTaskAction()} className="3xl:size-8 2xl:size-6.5 xl:size-5.5 lg:size-4.5 3xl:rounded-md 2xl:rounded-md xl:rounded-sm lg:rounded-sm border border-nav-highlight/20 bg-primary-shade-2 text-nav-highlight inline-flex items-center justify-center">
              <ExternalLink className="3xl:w-4 3xl:h-4 2xl:w-3.5 2xl:h-3.5 xl:w-3 xl:h-3 lg:w-2.5 lg:h-2" />
            </button>
            <button onClick={() => handleOpenStatusModal(row.original, TASK_MODAL_TYPES.INTERNAL)} className="3xl:size-8 2xl:size-6.5 xl:size-5.5 lg:size-4.5 rounded-md border border-nav-highlight/20 bg-background text-nav-highlight inline-flex items-center justify-center">
              <AiOutlineFileDone className="3xl:w-4 3xl:h-4 2xl:w-3.5 2xl:h-3.5 xl:w-3 xl:h-3 lg:w-2.5 lg:h-2" />
            </button>
          </div>
        ), size: getResponsiveSize({ lg: 69, xl: 92, '2xl': 104, '3xl': 130 }) },
  ...
  return (
    <div className="flex flex-col rounded-xl lg:rounded-2xl xl:rounded-3xl 2xl:rounded-4xl border border-table-stroke  overflow-hidden flex-1 min-h-0 h-full">
      <div className="3xl:px-4 2xl:px-3 xl:px-2 lg:px-1 pt-1 lg:pt-1.5 xl:pt-2 2xl:pt-2.5 3xl:pt-3 border-b border-table-stroke shrink-0 sticky top-0 z-20 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
