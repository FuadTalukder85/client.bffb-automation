...
import { Bug, Grip, CircleArrowRight } from "lucide-react";
...
import { UpdateProjectStageModal } from "./components/Details/UpdateProjectStageModal";
...
import { useApplicationLabDetailsLogic } from "./hooks/useApplicationLabDetailsLogic";
...
export default function ApplicationLabDetails() {
    const {
        projectId,
        projectResponseData,
        isLoading,
    ...
    const taskAutomation = useTaskAutomation({
        ...
        projectMembers,
        onInitiateUpdate: handleUpdateStage,
        onBack: () => setIsStageUpdateModalOpen(true),
    });

    const handleStageUpdateSubmit = (formData) => {
        const intercepted = taskAutomation.interceptStageUpdate(formData);
        if (!intercepted) {
            handleUpdateStage(formData);
        }
    };
            ...
            <div className=" flex flex-col items-center justify-center px-4 pb-20 mx-auto min-h-[60vh]">
                <div className="text-center">
                    ...
                    <p className="text-gray-500">
                        {queryError?.response?.data?.error?.message || queryError?.response?.data?.message || queryError?.message || "Project not found"}
                    </p>
                    <button
                        type="button"
                        onClick={handleBack}
    ...
    return (
        <section className=" flex flex-col px-0 md:pb-20  page-section-spacing min-h-[calc(100vh-6rem)]">
            <DetailsHeader
                ...
                handleOpenStageUpdateModal={handleOpenStageUpdateModal}
                isUpdatingStage={isUpdatingStage}
                actionButtons={[
                    {
                        icon: <CircleArrowRight className="w-4.5 lg:w-2.5 xl:w-3 2xl:w-3.5 3xl:w-4.5 h-4.5 lg:h-2.5 xl:h-3 2xl:h-3.5 3xl:h-4.5" />,
                        onClick: handleOpenStageUpdateModal,
            ...
            <div className="flex justify-between w-full gap-6 pt-2">
                <DetailsFieldGroups
                    ...
                    handleClearChildFields={handleClearChildFields}
            ...
            <UpdateProjectStageModal
                open={isStageUpdateModalOpen}
                onOpenChange={setIsStageUpdateModalOpen}
                onUpdate={handleStageUpdateSubmit}
