...
import { Bug, PanelLeft, Loader } from "lucide-react";
import { DetailsHeader } from "./components/Details/DetailsHeader";
import { InitiateProjectModal } from "./components/Details/InitiateProjectModal";
...
import { useMasterProjectDetailsLogic } from "./hooks/useMasterProjectDetailsLogic";
...
import { AssignTaskModal, TaskAssignedModal } from "../components/TaskAutomationModals";

export default function MasterProjectDetails() {
    const {
        projectId,
        projectResponseData,
        isLoading,
    ...
    const taskAutomation = useTaskAutomation({
        projectId,
        projectName: projectResponseData.masterProject?.title || "Project",
        projectMembers,
        onInitiateUpdate: handleInitiateUpdate,
        onBack: () => setIsInitiateModalOpen(true),
    });

    const handleInitiateSubmit = (formData) => {
        const intercepted = taskAutomation.interceptStageUpdate(formData);
        if (!intercepted) {
            handleInitiateUpdate(formData);
        }
    };
    ...
    const actionButtons = [
        // {
        //     icon: <Bug className="w-4.5 lg:w-2.5 xl:w-3 2xl:w-3.5 3xl:w-4.5 h-4.5 lg:h-2.5 xl:h-3 2xl:h-3.5 3xl:h-4.5" />,
        //     onClick: () => setShowDebugPanel(!showDebugPanel),
        //     label: "Debug",
        // },
        // initiate and export icon
        {
            icon: isExporting ? <Loader className="w-4.5 lg:w-2.5 xl:w-3 2xl:w-3.5 3xl:w-4.5 h-4.5 lg:h-2.5 xl:h-3 2xl:h-3.5 3xl:h-4.5 animate-spin" /> : <Upload className="w-4.5 lg:w-2.5 xl:w-3 2xl:w-3.5 3xl:w-4.5 h-4.5 lg:h-2.5 xl:h-3 2xl:h-3.5 3xl:h-4.5" />,
            onClick: handleExport,
            label: isExporting ? "Exporting" : "Export",
            disabled: isExporting,
        },
        {
            icon: <Rocket className="w-4.5 lg:w-2.5 xl:w-3 2xl:w-3.5 3xl:w-4.5 h-4.5 lg:h-2.5 xl:h-3 2xl:h-3.5 3xl:h-4.5" />,
    ...
    return (
        <section className="flex flex-col px-0 page-section-spacing">
            <DetailsHeader
                projectTitle={projectResponseData.masterProject?.title}
                handleBack={handleBack}
                handleExport={handleExport}
                handleInitiate={handleInitiateClick}
            ...
            <InitiateProjectModal
                ...
                onUpdate={handleInitiateSubmit}
                projectData={projectResponseData}
            />
            <AssignTaskModal
                open={taskAutomation.isAssignOpen}
                onOpenChange={taskAutomation.handleAssignBack}
                tasks={taskAutomation.tasksToCreate}
                projectMembers={projectMembers}
                onConfirm={taskAutomation.handleAssignConfirm}
                onBack={taskAutomation.handleAssignBack}
            />
            <TaskAssignedModal
                open={taskAutomation.isSuccessOpen}
                onClose={taskAutomation.handleSuccessClose}
                assignedTasks={taskAutomation.assignedTasksList}
                projectName={projectResponseData.masterProject?.title || "Project"}
            />
        </section>
    );
