...
export const DetailsFieldGroups = ({
  ...
  const renderField = (config) => {
    ...
    } else if (config.path === "productDevelopmentLab.tags") {
      if (Array.isArray(value)) {
        value = value
          .map((item) => {
            if (typeof item === "object" && item !== null) {
    ...
    } else if (config.path === "productDevelopmentLab.category" ||
      config.path === "productDevelopmentLab.subcategory" ||
      config.path === "productDevelopmentLab.subSubcategory") {
      if (value && typeof value === "object") {
        value = value.name || value.title || value._id || String(value);
      } else if (value) {
        value = String(value);
      }
  ...
  const renderGroup = (group, index) => {
    ...
    return (
      <div key={group[0]?.id || index} className="px-4 py-2 lg:py-[4.5px] xl:py-[5.5px] 2xl:py-[6.5px] 3xl:py-2">
