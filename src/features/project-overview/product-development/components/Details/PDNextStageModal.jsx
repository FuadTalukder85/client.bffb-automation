...
export function PDNextStageModal({ open, onOpenChange, onUpdate, projectData }) {
  const getDefaultAction = () => {
    const { common } = projectData || {};
    
    // For PDNextStageModal, the options are only "send_to_application_lab" and "not_feasible"
    // So we check if already sent to application, otherwise check if not feasible
    ...
    // PDNextStageModal only has 2 options, so default to send_to_application_lab
    return "send_to_application_lab";
  };

  const [formData, setFormData] = useState({
    action: "send_to_application_lab",
    brief: "",
  });
        ...
        <div className="grid gap-5 py-2">
          <div className="flex flex-col space-y-2 lg:space-y-0.5 xl:space-y-1 2xl:space-y-1.5 3xl:space-y-2">
            ...
            <Select
              ...
              className="w-full rounded-md bg-primary-shade-2"
