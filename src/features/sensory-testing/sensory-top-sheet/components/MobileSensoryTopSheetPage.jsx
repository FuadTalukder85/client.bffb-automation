...
export default function MobileSensoryTopSheetPage({
  searchTerm,
  handleSearchChange,
  statusOptions,
  selectedStatus,
  ...
  return (
    <div className="flex flex-col flex-1 w-full min-h-0">
      ...
      <div className="flex items-center justify-between py-4">
        <h1 className="text-heading font-bold">Sensory Topsheet</h1>
      </div>
                    ...
                    <ExpandableCard.Content initialHeight={130}>
                      ...
                      <InfoTable>
                        ...
                        <InfoTable.Row label="App Dev Status">
                          <StatusBadge label={project.appDevStatus} days={project.appDevDays} />
                        </InfoTable.Row>

                        <InfoTable.Row label="Sensory Status">
                          <StatusBadge label={project.sensoryStatus} days={project.sensoryDays} />
                        </InfoTable.Row>

                        <InfoTable.Row label="Purpose">
                          <div className="font-medium text-right text-muted-foreground truncate max-w-[180px]">
                            {project.purpose || "—"} ({project.purposeName || "—"})
                          </div>
                        </InfoTable.Row>
                          ...
                          <div className="flex flex-wrap gap-1 justify-end">
                            {project.tags?.length > 0 ? (
                              project.tags.map((tag, idx) => (
                                <span key={idx} className="bg-[#f0ebf8] text-[#7c5cc4] text-[9px] px-2 py-0.5 rounded-full border border-[#7c5cc4]/20 border-solid whitespace-nowrap uppercase font-bold tracking-tight">
                                  {tag}
                                </span>
                              ))
                            ) : (
