...
export default function SensoryTopSheetTable({
  data,
  currentPage,
  itemsPerPage,
  totalPages,
  ...
  const columns = useMemo(
    () => [
      ...
      {
        accessorKey: "appDevStatus",
        header: "Application Development Status",
        headerClassName: "table-head-cell text-center",
        cell: ({ row }) => (
          <StatusBadge 
            label={row.original.appDevStatus} 
            days={row.original.appDevDays} 
          />
        ),
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (!tags || tags.length === 0) return <span className="text-muted-foreground block text-center">N/A</span>;
          return (
            <div className="flex flex-wrap gap-1 justify-center px-2">
              {tags.map((tag, idx) => (
                <span key={idx} className="bg-[#f0ebf8] text-[#7c5cc4] text-[10px] px-2 py-0.5 rounded-full border border-[#7c5cc4]/20 border-solid whitespace-nowrap uppercase font-bold tracking-tight">
                  {tag}
                </span>
              ))}
            </div>
