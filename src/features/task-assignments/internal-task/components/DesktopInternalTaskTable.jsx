...
export function DesktopInternalTaskTable({
  tasks = [],
  isLoading,
  selectedFilter,
  searchTerm,
  ...
  const columns = [
    ...
    },
    {
      ...
      headerClassName: "table-head-cell",
      className: " ",
      cell: ({ getValue }) => <TruncatedText text={getValue()} className="font-medium text-nav-highlight" />,
      size: getResponsiveSize({ lg: 107, xl: 142, '2xl': 160, '3xl': 200 }),
    },
    {
      accessorKey: "team",
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex flex-col">
            <span className="font-medium text-nav-highlight truncate">{assignedTo?.name}</span>
            {assignedTo?.additional && <span className="text-muted-foreground truncate">{assignedTo.additional}</span>}
          </div>
        );
      },
      size: getResponsiveSize({ lg: 80, xl: 107, '2xl': 120, '3xl': 150 }),
          ...
          <div className="flex items-center justify-end gap-0 py-1 3xl:py-2.5">
            {task.isActive === false ? (
              <button
                ...
                className="action-button flex items-center justify-center gap-1.5 text-sm font-semibold rounded-md text-nav-highlight border-primary-shade-2 bg-primary-shade-2 hover:bg-primary-shade-2/80 transition-colors focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
              >
