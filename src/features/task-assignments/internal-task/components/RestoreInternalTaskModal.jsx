...
import { useRestoreInternalTask } from "@/hooks/mutations";

export function RestoreInternalTaskModal({
  open,
  onOpenChange,
  task,
  onConfirm,
  ...
  const { mutateAsync: restoreTask, isPending: isSubmitting } = useRestoreInternalTask();
    ...
    <Modal open={open} onOpenChange={handleClose}>
      ...
      >
        <ModalHeader className="flex flex-col items-center justify-center mb-4 space-y-4 text-center">
          ...
          >
            <div className="flex items-center justify-center rounded-full bg-primary text-white w-16 md:w-11 lg:w-13! xl:w-17! 2xl:w-19! 3xl:w-24! h-16 md:h-11 lg:h-13! xl:h-17! 2xl:h-19! 3xl:h-24!">
              <MdRestorePage className="w-10 md:w-5 lg:w-5.5! xl:w-7! 2xl:w-8! 3xl:w-10! h-10 md:h-5 lg:h-5.5! xl:h-7! 2xl:h-8! 3xl:h-10!" />
            </div>
          </motion.div>

          <ModalDescription className="text-sm md:text-[8px] lg:text-[8.5px]! xl:text-[11.5px]! 2xl:text-[13px]! 3xl:text-base! font-medium text-center text-lighter-text px-4">
