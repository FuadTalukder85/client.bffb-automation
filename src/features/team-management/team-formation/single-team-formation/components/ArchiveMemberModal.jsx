...
export function ArchiveMemberModal({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(err.message || "An error occurred during the archiving process");
    }
  };

  const displayTitle = title || "Archive Team Member";
  const displayMessage = message || (
    <>
      Are you sure you want to archive{" "}
      <span className="font-semibold text-foreground">{member?.name}</span> from the{" "}
      <span className="font-semibold text-foreground">{teamName}</span>?
    </>
  );
  const displaySubMessage = subMessage || "Current task progress will remain unchanged. Proceed with proper authorization.";
      ...
      >
        ...
        <div className="flex flex-col items-center space-y-4 md:space-y-6">
          ...
          <div className="flex flex-col items-center gap-2">
            <div className="px-4 md:px-4 lg:px-4.5! xl:px-5.5! 2xl:px-6.5! 3xl:px-8! text-center text-sm md:text-[8px] lg:text-[8.5px]! xl:text-[11.5px]! 2xl:text-[13px]! 3xl:text-base! font-medium text-base-color">
              {displayMessage}
            </div>
            <p className="text-center text-xs md:text-[7px] lg:text-[8px]! xl:text-[10px]! 2xl:text-xs! 3xl:text-sm! text-lighter-text">
              {displaySubMessage}
            </p>
          </div>
